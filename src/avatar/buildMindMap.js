...
export function buildPersonalizedMindMap({
  ...
  if (!attempts.length) {
    ...
    return buildEmptyTopicMap(topic || 'Science', prompt);
  }

  // Cap for layout (reference style works best with ~2–6 arms)
  const list = attempts.slice(0, 8);
  ...
  return {
    ...
    summary: `All ${totalMisses} incorrect answer${totalMisses === 1 ? '' : 's'} on one map — one card per miss.`,
    bigPicture:
      conceptCount > 1
    ...
    personalizedNote: `Every wrong answer is shown together (${totalMisses} miss${totalMisses === 1 ? '' : 'es'}).`,
...
function buildEmptyTopicMap(topic, prompt) {
  const color = BRANCH_COLORS[0];
  return {
    topic,
    root: topic,
    title: topic,
    summary: prompt || `Review ${topic}`,
    branches: [
      {
        id: 'miss-0',
        index: 1,
